...
test("file accept - image wildcard", function() {
	...
	validator = new $.validator({}, $form[0]),
	proxy = $.proxy($.validator.methods.accept, validator, null, input, "image/*");
...
test("file accept - specified mime type", function() {
	...
	validator = new $.validator({}, $form[0]),
	proxy = $.proxy($.validator.methods.accept, validator, null, input, "application/vnd.google-earth.kml+xml");
...
test("file accept - invalid mime type", function() {
	...
	var file = {
		name: "test.kml",
		size: 500001,
		type: "foobar/vnd.google-earth.kml+xml"
	}, fileList = {
		0: file,
		length: 1,
		item: dummy
